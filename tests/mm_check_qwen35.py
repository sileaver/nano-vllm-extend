...
def main():
    ref = torch.load(os.path.join(os.path.dirname(__file__), "ref_mm/img_prompt.pt"), weights_only=False)
    ...
    out = llm.generate([{"prompt": TEXT, "images": [os.path.join(os.path.dirname(__file__), "ref_mm/image.png")]}],
    ...
    out2 = llm.generate(
        [{"prompt": TEXT, "images": [os.path.join(os.path.dirname(__file__), "ref_mm/image.png")]},
         "Introduce yourself."],
        sampling_params=sp, use_tqdm=False)
